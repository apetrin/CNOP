...
class OrderedProbit(st.discrete.discrete_model.OrderedModel):
    ...
    def cons_generator(self, slice, dict_out=True):
        ...
        for st, fin in slice:
            for i in range(st, fin - 1):
                if dict_out:
                    constr.append({"type": "ineq",
                                   "fun": lambda x,
                                   i=i: np.array([float(x[i + 1] - x[i])])})
    ...
    def loglike(self, params):
        ...
        # FLOAT_EPS, 1)) for X,Y in izip(self.exog, self.endog) if Y ==
        # max(self.endog))

    def fit(self, start_params=None, method='COBYLA', maxiter=500,
            full_output=1, disp=1, callback=None, iprint=None, **kwargs):
        if start_params is None:
            start_params = list(
                self.get_start_params()) + range(max(self.endog))
        constraints = self.cons_generator(
            [(len(self.exog[0]), len(self.exog[0]) + max(self.endog))], dict_out=True)
    ...
    def get_start_params(self):
        ...
        return OLS(self.endog, self.exog).fit().params
        ...
        for X, Y in izip(self.exog, self.endog):
            ...
            if Y == 0:
                ...
                if dif == 0:
                    dif = FLOAT_EPS
                s1 = self.pdf(
                    mXb) / dif * np.append(np.append(-X, [1]), np.zeros(len(mu) - 1))
                s += s1
                ...
                s1 = - \
                    self.pdf(
                        mXb) / dif * np.append(np.append(-X, np.zeros(len(mu) - 1)), [1])
                s += s1
...
if __name__ == "__main__":
    ...
    pd.set_option(
        'display.mpl_style',
        'default')  # Make the graphs a bit prettier
